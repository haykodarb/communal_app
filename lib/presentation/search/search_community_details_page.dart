...
class SearchCommunityDetailsController extends GetxController {
  ...
  final CommonListViewController listViewController =
      CommonListViewController<Book>(pageSize: 20);
  ...
  Future<void> onInit() async {
    ...
    final BackendResponse<Membership> response =
        await UsersBackend.getMembershipByMemberAndCommunity(
      communityId: communityId,
...
class SearchCommunityDetailsPage extends StatelessWidget {
                ...
                FutureBuilder(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      ...
                      children: [
                        Expanded(
                          child: _communityInformation(community, controller),
                        ),
                        const Divider(height: 20),
                        Expanded(
                          child: _communityLibrary(community, controller),
                        ),
                      ],
                    );
                  },
  ...
  Widget _communityInformation(
      Community community, SearchCommunityDetailsController controller) {
    return Builder(
      builder: (context) {
        return Column(
              ...
              () {
                if (controller.existingMembership.value != null) {
                  final Membership membership =
                      controller.existingMembership.value!;
                  ...
                  if (!(admin_accepted!)) {
                    return const Text(
                        'Your request has been rejected by an administrator.');
                  }

                  if (!(member_accepted!)) {
                    return const Text(
                        'You have already rejected an invite to this community.');
                  }
                ...
                return ElevatedButton(
                  onPressed: () async {
                    ...
                    if (confirm) {
                      ...
                      final BackendResponse<Membership> response =
                          await UsersBackend.requestInviteToCommunity(
                              communityId);
  ...
  Widget _communityLibrary(
    Community community,
    SearchCommunityDetailsController controller,
  ) {
    return Container(
  ...
  Future<Community?> _findCommunity() async {
    ...
    community =
        searchPageController?.communityListController.itemList.firstWhereOrNull(
